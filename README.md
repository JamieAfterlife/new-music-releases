...
Saving requires a fine-grained GitHub token limited to the tracker repository with **Contents: Read and write** permission. On a personal phone or computer, select **Trust this personal device** during the first connection; the home-screen app or browser will then reconnect automatically. The token is encrypted using a non-exportable browser key and stays in that device's private site storage. On a shared device, leave that option off and use an unlock password of at least eight characters instead. **Forget saved token** removes either kind of saved connection, and no token or password is published with the site.
